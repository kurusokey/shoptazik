...
export default function CartPage() {
  ...
  if (checkoutDone) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-20 text-center" style={{ background: "#1A1610" }}>
        ...
        <p className="mt-4 text-white/40">Merci pour ta commande ! Tu recevras un e-mail de confirmation sous peu.</p>
        <Link href="/" className="mt-8 inline-block rounded-xl px-6 py-3 font-bold text-black transition hover:brightness-110" style={{ background: "linear-gradient(135deg, #C8A050, #A08030)" }}>
  ...
  if (items.length === 0) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-20 text-center" style={{ background: "#1A1610" }}>
        <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full" style={{ background: "rgba(255,255,255,0.05)" }}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="h-10 w-10 text-white/20">
        ...
        <p className="mt-2 text-white/40">Explore notre catalogue et ajoute des produits.</p>
        <Link href="/" className="mt-6 inline-block rounded-xl px-6 py-3 font-bold text-black transition hover:brightness-110" style={{ background: "linear-gradient(135deg, #C8A050, #A08030)" }}>
      ...
      <div className="grid gap-6 lg:grid-cols-3 lg:gap-8">
        ...
        <div className="h-fit rounded-xl border p-5" style={{ borderColor: "rgba(200,160,80,0.1)", background: "rgba(200,160,80,0.04)" }}>
          ...
          <div className="mt-4 space-y-2 border-b pb-4" style={{ borderColor: "rgba(200,160,80,0.1)" }}>
            {items.map((item) => (
              <div key={`${item.product.id}-${item.variant?.id}`} className="flex justify-between text-sm text-white/40">
                <span className="truncate pr-2">{item.product.name} x{item.quantity}</span>
                <span className="shrink-0">{formatPrice((item.product.price + (item.variant?.price_modifier ?? 0)) * item.quantity)}</span>
              </div>
            ))}
          </div>
